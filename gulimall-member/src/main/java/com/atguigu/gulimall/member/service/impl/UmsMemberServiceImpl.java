...
import com.atguigu.gulimall.member.dao.UmsMemberLevelDao;
...
public class UmsMemberServiceImpl extends ServiceImpl<UmsMemberDao, UmsMemberEntity> implements UmsMemberService {
    ...
    public void checkPhoneUnique(String phone) throws PhoneException {
        ...
        Integer phoneCount = this.baseMapper.selectCount(new QueryWrapper<UmsMemberEntity>().eq("mobile", phone));
    ...
    public void checkUserNameUnique(String userName) throws UsernameException {
        ...
        Integer usernameCount = this.baseMapper.selectCount(new QueryWrapper<UmsMemberEntity>().eq("username", userName));
    ...
    @Override
    public UmsMemberEntity login(MemberUserLoginVo vo) {

        String loginacct = vo.getLoginacct();
        String password = vo.getPassword();

        //1、去数据库查询 SELECT * FROM ums_member WHERE username = ? OR mobile = ?
        UmsMemberEntity memberEntity = this.baseMapper.selectOne(new QueryWrapper<UmsMemberEntity>()
    ...
    @Override
    public UmsMemberEntity login(SocialUser socialUser) throws Exception {

        //具有登录和注册逻辑
        String uid = socialUser.getUid();

        //1、判断当前社交用户是否已经登录过系统
        UmsMemberEntity memberEntity = this.baseMapper.selectOne(new QueryWrapper<UmsMemberEntity>().eq("social_uid", uid));
    ...
    @Override
    public UmsMemberEntity login(String accessTokenInfo) {
        ...
        UmsMemberEntity memberEntity = this.baseMapper.selectOne(new QueryWrapper<UmsMemberEntity>().eq("social_uid", openid));
        ...
        if (memberEntity == null) {
            ...
            //把扫码人的信息添加到数据库中
            memberEntity = new UmsMemberEntity();
